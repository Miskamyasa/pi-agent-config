 ...
 * Configuration via <agentDir>/extensions/mnemosyne/config.json (global)
 * merged with <cwd>/.pi/mnemosyne.json (project, trusted only; project
 * wins per key), or environment (MNEMOSYNE_URL, MEMORY_MCP_TOKEN,
 * MNEMOSYNE_INSECURE=1). Bank scoping maps to mnemosyne memory banks: the
 * global bank is always named "global"; the project bank is "project--"
 * plus the optional project-config "bank" override (e.g. "pi" →
 * project--pi) or the cwd path (e.g. /Users/dzaitsev/.pi →
 * project--Users-dzaitsev--pi).
 * "bankScope" is "global" (global bank only), "project" (project bank
 * only), or "hybrid" (default: reads fan out over both, writes default to
 * the project bank, and the mnemosyne_memory add action can target
 * "global").
 */
import {
  getMarkdownTheme,
...
export default function mnemosyneExtension(pi: ExtensionAPI) {
  ...
  let banks: BankSet = resolveBanks({ bankScope: "global" }, "");
  let topK = 5;
